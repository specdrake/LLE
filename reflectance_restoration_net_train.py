...
#decomnet input
input_all = tf.compat.v1.placeholder(tf.float32, [None, None, None, 3], name='input_all')
...
loss_ssim = 1-ssim_r
#mse loss
loss_square = tf.reduce_mean(input_tensor=tf.square(output_r  - input_high))#*(1-input_low_i))# * ( 1 - input_low_r ))#* (1- input_low_i)))
#total loss
...
global_step = tf.compat.v1.get_variable('global_step', [], dtype=tf.int32, initializer=tf.compat.v1.constant_initializer(0), trainable=False)
