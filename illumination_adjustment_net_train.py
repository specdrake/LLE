...
input_decom = tf.compat.v1.placeholder(tf.float32, [None, None, None, 3], name='input_decom')
...
def grad_loss(input_i_low, input_i_high):
    ...
    grad_loss_all = tf.reduce_mean(input_tensor=x_loss + y_loss)
    return grad_loss_all

loss_grad = grad_loss(output_i, input_high_i)
loss_square = tf.reduce_mean(input_tensor=tf.square(output_i  - input_high_i))
