...
input_low = tf.compat.v1.placeholder(tf.float32, [None, None, None, 3], name='input_low')
input_high = tf.compat.v1.placeholder(tf.float32, [None, None, None, 3], name='input_high')
...
def mutual_i_loss(input_I_low, input_I_high):
    ...
    mutual_loss = tf.reduce_mean( input_tensor=x_loss + y_loss) 
    return mutual_loss
...
def mutual_i_input_loss(input_I_low, input_im):
    ...
    x_loss = tf.abs(tf.math.divide(low_gradient_x, tf.maximum(input_gradient_x, 0.01)))
    low_gradient_y = gradient(input_I_low, "y")
    input_gradient_y = gradient(input_gray, "y")
    y_loss = tf.abs(tf.math.divide(low_gradient_y, tf.maximum(input_gradient_y, 0.01)))
    mut_loss = tf.reduce_mean(input_tensor=x_loss + y_loss) 
    return mut_loss

recon_loss_low = tf.reduce_mean(input_tensor=tf.abs(R_low * I_low_3 -  input_low))
recon_loss_high = tf.reduce_mean(input_tensor=tf.abs(R_high * I_high_3 - input_high))

equal_R_loss = tf.reduce_mean(input_tensor=tf.abs(R_low - R_high))
