...
def gradient_no_abs(input_tensor, direction):
    ...
    smooth_kernel_y = tf.transpose(a=smooth_kernel_x, perm=[1, 0, 2, 3])
    if direction == "x":
        kernel = smooth_kernel_x
    elif direction == "y":
        kernel = smooth_kernel_y
    gradient_orig = tf.nn.conv2d(input=input_tensor, filters=kernel, strides=[1, 1, 1, 1], padding='SAME')
    grad_min = tf.reduce_min(input_tensor=gradient_orig)
    grad_max = tf.reduce_max(input_tensor=gradient_orig)
    grad_norm = tf.math.divide((gradient_orig - grad_min), (grad_max - grad_min + 0.0001))
    return grad_norm
...
def gradient(input_tensor, direction):
    ...
    if direction == "x":
        ...
        kernel = smooth_kernel_y
    gradient_orig = tf.abs(tf.nn.conv2d(input=input_tensor, filters=kernel, strides=[1, 1, 1, 1], padding='SAME'))
    grad_min = tf.reduce_min(input_tensor=gradient_orig)
    grad_max = tf.reduce_max(input_tensor=gradient_orig)
    grad_norm = tf.math.divide((gradient_orig - grad_min), (grad_max - grad_min + 0.0001))
    return grad_norm
...
def blur(x):
    ...
    return tf.nn.depthwise_conv2d(input=x, filter=kernel_var, strides=[1, 1, 1, 1], padding='SAME')
